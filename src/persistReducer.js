...
export function persistReducer<State: Object, Action: Object>(
  ...
  return (state: State, action: Action) => {
    ...
    let restState: State = state ? rest : undefined;

    console.log('persist', state, restState);
    switch (action.type) {
      case PERSIST:
        if (state._persist) {
          console.warn(
        ...
        getStoredState(config, (err, restoredState) => {
          ...
          action.rehydrate(restoredState, err, config.key);
        });
      ...
      default:
        ...
        console.log('default', restState, newState);
        _persistoid && _persistoid.update(state);
        return newState;
    }
  };
}
